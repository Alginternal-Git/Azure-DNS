...
```markdown
# 🌐 Azure DNS Deployment with Terraform & GitHub Actions

This project automates the provisioning of an **Azure DNS** using Terraform. It integrates with **GitHub Actions** to create a CI/CD pipeline that applies infrastructure changes upon pushing to the `main` branch. Also displays the cost of resources used using INFRACOST and creates the resources only after the approval from reviewers.
...
```bash
terraform apply \
  -var="resource_group_name=DNSresources" \
  -var="resource_group_location=West Europe" \
  -var="dns_zone_name=mydomain.com" \
  -var="public_ip_name=mypublicip" \
  -var="public_ip_allocation_method=Dynamic" \
  -var="public_ip_version=IPv4" \
  -var="dns_a_record_name=ARecord" \
  -var="dns_a_record_ttl=300"
...
| `DNS Zone`                | Generated DNS Zone                                   |
| `DNS A Record`            | Generated DNS A Record                               |
...
* [Azure DNS creation using terraform](https://learn.microsoft.com/en-us/azure/dns/dns-get-started-terraform?tabs=azure-cli)
